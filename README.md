...
一个以中文物理长文为核心的 Astro 静态博客。首页保持极简，正文采用纸刊式三栏阅读，趣味功能集中在可关闭的“书房”中。
...
- 系统/浅色/深色主题、首页纸面纹理、站点统计和书房侧栏
- 评论与音乐采用配置开关；缺少配置时自动隐藏
...
站名、作者、地点和功能开关集中在 `src/config/site.ts`。正式域名确定后，用 `SITE_URL` 环境变量覆盖默认地址。
